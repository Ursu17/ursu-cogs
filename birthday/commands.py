...
class BirthdayCommands(MixinMeta):
    ...
    async def set(self, ctx: commands.Context, *, birthday: BirthdayConverter):
        ...
        Data de nastere e sub forma de ZZ-LL-AAAA.
        ...
        **Exemple:**
        ...
        if birthday.year != 1 and birthday.year < MIN_BDAY_YEAR:
            await ctx.send(f"Imi pare rau de nu pot adauga data de nastere inainte de {MIN_BDAY_YEAR}.")
            return

        if birthday > datetime.datetime.utcnow():
            await ctx.send("Nu te poti naste in viitor. Ce plm ai baut!")
            return
        ...
        if birthday.year == 1:
            str_bday = birthday.strftime("%d %B")
        else:
            str_bday = birthday.strftime("%d %B, %Y")

        await ctx.send(f"Data ta de nastere a fost setata pe {str_bday}.")

    @birthday.command(aliases=["delete", "del"])
    async def remove(self, ctx: commands.Context):
        """Sterge data nasterii."""
        # guild only check in group
        if TYPE_CHECKING:
            assert isinstance(ctx.author, discord.Member)
            assert ctx.guild is not None

        m = await ctx.send("Esti sigur(a)?")
        ...
        if check.result is False:
            await ctx.send("Anulat.")
            return

        await self.config.member(ctx.author).birthday.set({})
        await ctx.send("Data ta de nastere a fost stearsa.")
    ...
    async def upcoming(self, ctx: commands.Context, days: int = 7):
        ...
        **Examples:**
        ...
        if len(parsed_bdays) == 0:
            await ctx.send(f"Nu sunt zile de nastere programate in urmatoarele {days} zile.")
            return

        sorted_parsed_bdays = sorted(parsed_bdays.items(), key=lambda x: x[0])

        embed = discord.Embed(title="Zile de nastere viitoare", colour=await ctx.embed_colour())

        if len(sorted_parsed_bdays) > 25:
            embed.description = "Prea multe zile de calculat. A trebuit sa ma opresc la 25."
